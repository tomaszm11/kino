...
db=psycopg2.connect(host='localhost',user='user',password='your password',database='kino')
c=db.cursor()
...
class Start(): 
    ...
    def __init__(self,master): 
        ...
        self.seans_btn_start=Button(self.start_frame,text='Seanse',command=self.switch_to_seanse)
...
class Admin():
    def __init__(self,master):
        ...
        self.lbl_admin=Label(self.admin_frame,text='tu cos bedzie').pack()
...
class Accept():
    ...
    def accept(self,res):
        ...
        exec(f'self.lbl_accept_p_id{res[0]}.destroy()')
        exec(f'self.lbl_accept_p_seats{res[0]}.destroy()')
        exec(f'self.lbl_accept_p_idsea{res[0]}.destroy()')
        exec(f'self.lbl_accept_free_seats{res[0]}.destroy()')
        self.button_dict[f'btn_accept{res[0]}'].destroy()
...
class Add_admin():
    def __init__(self,master):
        ...
        self.e_surname.pack()
        self.lbl_id=Label(self.add_admin_frame,text='Wpisz id nowego pracownika').pack()
...
class Seanse():
    ...
    def show_seans(self,title):
        ...
        for k in data:
            ...
            self.count1+=1
        self.btn_show_seans_back=Button(self.show_seanse_frame,text='Powrot',command=self.sh_back).grid(row=134,column=2)
    def reserve(self,id):
        
        exec(f'self.seats{id}=self.e_seats_{id}.get()')
        a=[]
        exec(f'a.append(self.seats{id})')
